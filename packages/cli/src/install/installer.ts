...
// Keep in sync with generatedStyleFileHeader in packages/tokens generator.config.ts
const generatedStyleFileHeader = `/**
 * AUTO-GENERATED FILE.
 *
  ...
  for (const style of styles) {
    ...
    for (const file of style.files) {
      ...
      if (await fileExists(targetPath)) {
        ...
        console.log(
          `Conflict: style already exists and differs: ${targetPath}`,
        );
        result.conflicted.push(targetPath);
        continue;
      }
