...
- CSS variables follow: `--nx-*-destructive-*` → `--nx-*-danger-*`.
- Semantic source remains `action.danger`.

### Non-breaking notes

- Toast still uses Base UI `type: "destructive"` on the DOM (`data-[type=destructive]`); Neurex tokens use `danger` naming.
- Shared disabled/busy opacity uses `--nx-opacity-disabled` and `--nx-opacity-busy`.
